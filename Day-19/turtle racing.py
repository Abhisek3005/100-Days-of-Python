import random
import turtle
from  turtle import *

screen = Screen()
screen.setup(width = 800, height=700)
user_bet = screen.textinput(title="Make your Bet" , prompt="Which turtle will win the race? Enter the turtle color:")
print(user_bet)

colors = ["red" , "orange", "green" , "blue" , "purple" , "yellow", "black" , "navy" ]
y_position = [300, 200, 100, 0 ,-100, -200, -300]
all_turtle = []
is_race_on = False
for turtle_index in range(0,7):
 new_turtle  =Turtle(shape ="turtle")
 new_turtle.color(colors [turtle_index])
 new_turtle.penup()
 new_turtle.goto(x = -380, y = y_position[turtle_index])
 all_turtle.append(new_turtle)

 if user_bet:
  is_race_on = True



while is_race_on :
    for turtlee in all_turtle:
        if turtlee.xcor() > 380:
         is_race_on = False
         winning_color = turtlee.pencolor()
         if winning_color == user_bet:
             print(f"You have won! The {winning_color} turtle is the winner")
         else:
            print(f"You have lost! The {winning_color} turtle is the winner")
        rand_distance = random.randint(0,10)
        turtlee.forward(rand_distance)







screen.exitonclick()
